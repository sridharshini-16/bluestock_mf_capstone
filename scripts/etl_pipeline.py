...
print(f"  → {len(df_fund)} rows saved")
...
print(f"  → Source rows: {before}, After fill: {len(df_nav_filled)} rows saved")
print(f"  → NAV > 0 validation: PASSED (min NAV = {df_nav_filled['nav'].min():.4f})")
...
print(f"  → {len(df_folio)} rows saved")
...
print(f"  → {len(df_perf)} rows saved")
...
print(f"  → {len(df_port)} rows saved")
...
print(f"  → {len(df_bench)} rows saved")

print("\n" + "=" * 60)
print("STEP 2: LOADING INTO SQLITE")
print("=" * 60)
...
with engine.connect() as conn:
    for table_name, df in table_map.items():
        ...
        print(f"  [{match}] {table_name}: source={src_rows}, db={result}")

print("\nAll tables loaded into:", DB_PATH)
print("\n" + "=" * 60)
print("DONE — Day 2 pipeline complete.")
print("=" * 60)
